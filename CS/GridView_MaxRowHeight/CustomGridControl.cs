...
namespace CustomGridColumn_MaxRowHeight
{
    ...
    public class CustomGridViewInfo : GridViewInfo
    {
        ...
        protected override int CalcRowAutoHeight(System.Drawing.Graphics g, GridColumnsInfo colInfo, int rowHandle, bool useCache, int level)
        {
            if (((CustomGridView)View).OptionsView.MaxRowHeight > 0)
                return Math.Min(((CustomGridView)View).OptionsView.MaxRowHeight, base.CalcRowAutoHeight(g, colInfo, rowHandle, useCache, level));
            return base.CalcRowAutoHeight(g, colInfo, rowHandle, useCache, level);
        }
    }
